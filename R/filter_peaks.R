...
#'
#' @param PeakData (peak_data object) object generated by get_peak_data (from mzML or raw),
#'     or make_peak_data in pspecterlib. Required.
#' @param MZRange (numeric) A range of MZ values to filter the data by.
#'     It is highly recommended that users visualize the spectra first to determine
#'     a reasonable cutoff range. Required.
#' @param NoiseFilter (numeric) An abundance (every peak is scaled to the largest peak) cutoff
#'     for peaks. A reasonable value should be in the 0.01 - 5.0% range. Default is 1%.
#'
#' @returns (peak_data object) A peak_data object that has been filtered.
#'
#' @examples
#' \dontrun{
#'
